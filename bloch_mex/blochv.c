...
int blochsim(double *b1real, double *b1imag, 
		...
		double dx, double dy, double dz, double dvx, double dvy, double dvz,
    ...
    for (poscount=0; poscount < npos; poscount++)
	{
		...
		if (mode == 3)	/* Steady state AND record all time points. */
			...
			blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
				...
				*dzptr, *dvxptr, *dvyptr, *dvzptr, mx, my, mz, 1);
			/*TJ added dvxptr*/
			blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
				e1, e2, *dfreq, *dxptr, *dyptr, 
				*dzptr, *dvxptr++, *dvyptr++, *dvzptr++, mx, my, mz, 2);
			}
		else
			{
			/*TJ added dvxptr*/
			blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
				e1, e2, *dfreq, *dxptr, *dyptr, 
				*dzptr, *dvxptr++, *dvyptr++, * dvzptr++, mx, my, mz, mode);
			}
...
blochsimfz(b1r,b1i,gx,gy,gz,tp,ntime,t1,t2,df,nf,dx,dy,dz,npos,dvx,dvy,dvz,nvel,mx,my,mz,md);


/* ======= Reshape Output Matrices ====== */

if ((ntout > 1) && (nf > 1) && (npos > 1) && (nvel > 1))
	{
...
else if ( ( (int)(ntout>1) + (int)(ntout>1) + (int)(ntout>1) + (int)(ntout>1) )	== 3 )		/* Basically "squeeze" the matrix. */
	{
...
else if ( ( (int)(ntout>1) + (int)(ntout>1) + (int)(ntout>1) + (int)(ntout>1) )	<= 2 )
{
#ifdef DEBUG
	...
	if (ntout > 1)	
	...
	else
		{
			if ( npos > 1)
				...
				{
				#ifdef DEBUG
  					printf("Writing output, only nvel and nf.\n");
				#endif
				outsize[0]=nvel;
				outsize[1]=nf;		
				}
