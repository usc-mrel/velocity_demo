...
%	Bloch simulator.
%
%	[mx,my,mz] = bloch_asl(b1,gr,tp,t1,t2,df,dp,mode,
%       mbf,pcoeff,t1bld,eff,delt,bt,labType,ts,labStamp,mx,my,mz)
%
...
%
%       mbf = simulated myocardial blood flow (ml/g/min)
%       pcoeff = partition coefficient (.92)
%       tlbld = T1 of blood
%       eff = labeling efficiency 
%       delt = transit time
%       bt = bolus width
%       labType = 0 for saturation, 1 for inversion ? or other way around
%       ts = forgot... when you are performing imaging (note, this is
%       flexible for spasl, so you image and label once per heartbeat and
%       you can continue this simulation for multiple heart beats)
%       labStamp = when you are performing labeling 
%
%	(optional)
