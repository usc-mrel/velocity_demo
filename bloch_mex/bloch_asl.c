...
void blochsim(double *b1real, double *b1imag, 
		...
		int ntime, double *e1, double *e2, double *aslSig, double df, 
		double dx, double dy, double dz, 
		double *mx, double *my, double *mz, int mode, double M0)
...
/* genASLsig will be off because it assumes labeling occurs at t=0, we need to pass time when labeling occured */
void genASLSig(double *aslSig, double *tsteps, int ntime, double flow, double pcoeff, double k, double t1app, double t1bld, double eff, double delt, double bt, double labType, double ts, double *labStamp, int nLabel)
{
int count;
int lcount; 
...
e1bld = (double *) malloc(ntime * sizeof(double));
ek = (double *) malloc(ntime * sizeof(double));
e1app = (double *) malloc(ntime * sizeof(double));



if(labType > 0) eff*=2;         /* labType = 0 is saturation, otherwise inversion - not accurate, doesn't account spin history*/

for (lcount = 0; lcount < nLabel; lcount++)
    {
    
	if (labStamp[lcount] < ts && ts-labStamp[lcount] < 3*t1bld)
		{
		aslSigptr = aslSig;
		e1bldptr = e1bld;
		ekptr = ek;
		...
		/*printf( "labStamp[%d] = %0.6f\t nLabel = %d, k = %0.6f\n", lcount, labStamp[lcount], nLabel, k);*/
		for (count=0; count < ntime; count++)
			{
				*e1bldptr = exp( -(*tstepsptr) / t1bld);
				*ekptr    = exp( (*tstepsptr) / k);
				*e1appptr    = exp( -(*tstepsptr) / t1app);

				if(*tstepsptr*count+ts-labStamp[lcount] < delt)
				{   
					/*printf("case 1: less than transit time\n");*/
					*aslSigptr += 0;
				}
				else if ( (*tstepsptr*count+ts-labStamp[lcount] >= delt ) && (*tstepsptr*count+ts-labStamp[lcount] < delt + bt) )
				{
					/*printf("case 2: Between transit time and bolus width\n");*/
					/**aslSigptr += flow/pcoeff * eff * *e1bldptr * *ekptr * ( exp(-k*delt) - *ekptr )/k;*/
					/**aslSigptr += flow/pcoeff * eff * *e1bldptr * (*ekptr - 1 )/k;*/
					/**aslSigptr += flow/pcoeff * eff * *e1bldptr * *tstepsptr;*/
					*aslSigptr += flow/pcoeff * eff * *e1bldptr * *e1appptr * *tstepsptr;
					/**aslSigptr += flow/pcoeff * eff * *tstepsptr;*/
				}
				else 
				{
					/*printf("case 3: Greater than bolus width\n");*/
					/**aslSigptr += flow/pcoeff * eff* *e1bldptr * *ekptr * ( exp(-k*delt) - exp(-k*(delt+bt)) )/k;*/
					/**aslSigptr += flow/pcoeff * eff* *e1bldptr * *ekptr /k;*/
					*aslSigptr += 0;
				}
	...
	/*printf( "time = %0.6f\taslSig = %0.6f\te1bld = %0.6f\tek = %0.6f\n\tk = %0.6f\tflow = %0.6f\tpcoeff = %0.6f\n", *--tstepsptr, *--aslSigptr, *--e1bldptr, *--ekptr, k, flow, pcoeff);*/
}
...
void blochsimfz(double *b1real, double *b1imag, double *xgrad, double *ygrad, double *zgrad, 
		...
		double *mx, double *my, double *mz, int mode, double flow, double pcoeff, 
		double t1bld, double eff, double delt, double bt, double labType, double ts, double *labStamp, int nLabel, double M0)
/*Adding flow effects
flow    =  tissue blood flow (ml/g/min)
pcoeff  =  water blood partition coefficient 
t1blood =  t1 of blood
delt    =  transit delay
bt      =  bolus duration
labType =  0 saturation, 1 inversion
*/
...
/* TJ additional Variables */
double t1app;      /*T1 apparent (includes flow effects)*/
double k;          /*1/T1blood - 1/T1app used by detre kinetic model description of ASL*/
double *aslSig; 
...
aslSig = (double *) calloc(ntime, sizeof(double));
...
/* TJ Addition*/
t1app = 1/( 1/t1 + flow/pcoeff);
k = 1/t1bld - 1/t1app;
/*printf("t1app = %0.4f\t k = %0.4f\n", t1app, k);*/
/*End TJ*/

for (count=0; count < ntime; count++)
	{
	*e1ptr++    = exp(- *tstepsptr / t1app);       /*TJ modification*/
...
/*Generate ASL signal*/
/*printf("GenASlSig\n");*/
genASLSig(aslSig, tsteps, ntime, flow, pcoeff, k, t1app, t1bld, eff, delt, bt, labType, ts, labStamp, nLabel);
...
for (fcount=0; fcount < nfreq; fcount++)
	...
	if (mode == 3)	/* Steady state AND record all time points. */
		...
		blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
			e1, e2, aslSig, *dfreq, *dxptr, *dyptr, 
			*dzptr, mx, my, mz, 1, M0);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
	...
	/* bloch(b1,gradxyz,dt,t1,t2, df,dx,dy,dz,mode,mx,my,mz, flow,pcoeff,t1bld,eff,delt,bt,labType) */
{
...
double flow;
double pcoeff; 
double t1bld;   /*T1 blood*/
double eff;     /*Labeling Efficiency*/
double delt;    /* Transit Delay */
double bt;      /*Bolus Duration*/
double labType; /*Labeling Type 0 Saturation, 1 Inversion */

double ts;      /*Time Stamp - need to know where we are relative to ASL labeling history*/
double *labStamp; /*When ASL labeling occurs*/

int     nLabel;     /*Number of ASL labeling pulses*/
...
/* ===== Flow Variables ===== */
flow = *mxGetPr(prhs[7]);
flow = flow/60;					/*Convert from ml/g/min to ml/g/sec*/
pcoeff = *mxGetPr(prhs[8]);
t1bld = *mxGetPr(prhs[9]);
eff = *mxGetPr(prhs[10]);
delt = *mxGetPr(prhs[11]);
bt = *mxGetPr(prhs[12]);
labType = *mxGetPr(prhs[13]);
ts = *mxGetPr(prhs[14]); 

nLabel = mxGetM(prhs[15]) * mxGetN(prhs[15]);
labStamp = mxGetPr(prhs[15]);
M0 = *mxGetPr(prhs[16]);	

if (nrhs > 17)
	md = (int)(*mxGetPr(prhs[17]));		
else
	md = 0;
...
if ( (nrhs > 20) &&	 
	(mxGetM(prhs[18]) * mxGetN(prhs[18]) == nfnpos) &&
        (mxGetM(prhs[19]) * mxGetN(prhs[19]) == nfnpos) &&
        (mxGetM(prhs[20]) * mxGetN(prhs[20]) == nfnpos)  )
	...
	mxin = mxGetPr(prhs[18]);
	myin = mxGetPr(prhs[19]);
	mzin = mxGetPr(prhs[20]);
	...
	{
	#ifdef DEBUG
	if (nrhs > 20) 	 /* Magnetization given, but wrong size! */
		{
...
blochsimfz(b1r,b1i,gx,gy,gz,tp,ntime,t1,t2,df,nf,dx,dy,dz,npos,mx,my,mz,md,flow,pcoeff,t1bld,eff,delt,bt,labType,ts,labStamp,nLabel,M0);
