...
int blochsim(double *b1real, double *b1imag, 
		...
		double dx, double dy, double dz, double dvx, double dvy, double dvz, double db1, 
...
for (tcount = 0; tcount < ntime; tcount++)
	...
	/*TJ*/
    /*Velocity Simulation*/
    dxt = dx + dvx * *tsteps * (float)tcount;
    dxt = dy + dvx * *tsteps * (float)tcount;
    dxt = dz + dvx * *tsteps * (float)tcount;

    gammadx = dxt*GAMMA;	/* Convert to Hz/cm */
    gammady = dyt*GAMMA;	/* Convert to Hz/cm */
    gammadz = dzt*GAMMA;	/* Convert to Hz/cm */
	...
	rotz = -(*xgrad++ * gammadx + *ygrad++ * gammady + *zgrad++ * gammadz +
								df*TWOPI ) * *tsteps;
	rotx = (- db1 * *b1real++ * GAMMA * *tsteps);
	roty = (+ db1 * *b1imag++ * GAMMA * *tsteps++);
...
double *dvxptr, *dvyptr, *dvzptr; /*TJ*/
double *dfreqptr; 				  /*TJ*/
...
totpoints = npos*nfreq*nvel*nb1; /*TJ*/

/*TJ*/
for (b1count=0; b1count < nb1; b1count++)
{
	dfreqptr = dfreq;	
	...
	for (fcount=0; fcount < nfreq; fcount++)
		...
		for (poscount=0; poscount < npos; poscount++)
		{
			...
			for (velcount=0; velcount < nvel; velcount++)
			{

			/*#if DEBUG
			printf("dxptr = %0.2f\t dvxptr = %0.2f\n", *dxptr, *dvxptr);
			printf("dyptr = %0.2f\t dvyptr = %0.2f\n", *dyptr, *dvyptr);
			printf("dzptr = %0.2f\t dvzptr = %0.2f\n", *dzptr, *dvzptr);
			#endif*/

			/*!!!!!! BUG HERE DUNNO HOW TO INCLUDE VARIOuS VELOCITIES HERE!!!!!*/
			if (mode == 3)	/* Steady state AND record all time points. */
				...
				blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
					e1, e2, *dfreqptr, *dxptr, *dyptr, 
					*dzptr, *dvxptr, *dvyptr, *dvzptr, *db1, mx, my, mz, 1);
				/*TJ added dvxptr*/
				blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
					e1, e2, *dfreqptr, *dxptr, *dyptr, 
					*dzptr, *dvxptr++, *dvyptr++, *dvzptr++, *db1, mx, my, mz, 2);
				}
			else
				{
				/*TJ added dvxptr*/
				blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
					e1, e2, *dfreqptr, *dxptr, *dyptr, 
					*dzptr, *dvxptr++, *dvyptr++, * dvzptr++, *db1, mx, my, mz, mode);
				}
...
/*TJ*/
double *dvx;	/* X Velocity (cm/s)			*/
double *dvy;	/* Y Velocity (cm/s)			*/
double *dvz;	/* Z Velocity (cm/s)			*/
double *db1; /* b1 scale					*/
...
/*TJ*/
int dvyaflag=0; /*1 if dvy was allocated */
int dvzaflag=0; /*1 if dvz was allocated */

int ntime;	/* Number of time points. 	 */
int ntout;	/* Number of time poitns at output. */
int outsize[5];	/* Output matrix sizes		*/
...
/*TJ*/
int nfnposnvel; /* Number of frequencies * number of positions * velocities */
int nfnposnvelnb1; /*Number of freq * pos * vel * b1scale */

int ntnfnposnvel; /* Number of output times * frequencies * number of positions * velocities */
int ntnfnposnvelnb1; /*Number of output times * freq * pos * vel * b1scale */
/*End TJ*/
...
#if DEBUG
  printf("Velocity vector is %d x %d. \n",nvelM,nvelN);
#endif

if (nvelN==3)			/* Assume 3 position dimensions given */
	{
	nvel = nvelM;
	#if DEBUG
	  printf("Assuming %d 3-Dimensional Velocities\n",nvel);
	#endif
	dvx = mxGetPr(prhs[7]);
	dvy = dvx + nvel;
	dvz = dvy + nvel;
...
else if (nvelN==2)		/* Assume only 2 position dimensions given */
	{
	nvel = nvelM;
	#if DEBUG
	  printf("Assuming %d 2-Dimensional Velocities\n",nvel);
	#endif
	dvx = mxGetPr(prhs[7]);
	dvy = dvx + nvel;
	dvz = (double *)malloc(nvel * sizeof(double));
	dvzaflag=1;
	...
	nvel = nvelM * nvelN;
	#if DEBUG
	  printf("Assuming %d 1-Dimensional Velocities\n",nvel);
	#endif
	dvx = mxGetPr(prhs[7]);
	dvy = (double *)malloc(nvel * sizeof(double));
	dvz = (double *)malloc(nvel * sizeof(double));
	dvyaflag=1;
	dvzaflag=1;
	for (count=0; count < nvel; count++)
...
if (dvx == NULL) 
	printf("ERROR:  dvx is not allocated. \n");
if (dvy == NULL) 
	printf("ERROR:  dvy is not allocated. \n");
if (dvz == NULL) 
	printf("ERROR:  dvz is not allocated. \n");

nfnposnvel = nf*npos*nvel;	/* Just used to speed things up below. 	*/ 
...
db1 = mxGetPr(prhs[8]);
nb1 = mxGetM(prhs[8]) * mxGetN(prhs[8]);

nfnposnvelnb1 = nf*npos*nvel*nb1; 

#if DEBUG
  printf("%d B1 scale points.\n",nb1);
#endif
/*End TJ*/
...
/*TJ changed from 7 to 9*/
if (nrhs > 9)
	md = (int)(*mxGetPr(prhs[9]));		
else
	md = 0;
...
/*TJ*/ ntnfnposnvel = ntout*nfnposnvel;
/*TJ*/ ntnfnposnvelnb1 = ntout*nfnposnvelnb1;
...
plhs[0] = mxCreateDoubleMatrix(ntnfnposnvelnb1,1,mxREAL);	/* Mx, output. */
plhs[1] = mxCreateDoubleMatrix(ntnfnposnvelnb1,1,mxREAL);	/* My, output. */
plhs[2] = mxCreateDoubleMatrix(ntnfnposnvelnb1,1,mxREAL);	/* Mz, output. */
...
/* TJ increase all indices by 2, change nfnpos to nfnposnvel*/
/*Tj change from nfnposnvel to nfnposnvelnb1*/
if ( (nrhs > 12) &&	 
	(mxGetM(prhs[10]) * mxGetN(prhs[10]) == nfnposnvelnb1) &&
        (mxGetM(prhs[11]) * mxGetN(prhs[11]) == nfnposnvelnb1) &&
        (mxGetM(prhs[12]) * mxGetN(prhs[12]) == nfnposnvelnb1)  )
	...
	/* TJ increase all indices by 1, change nfnpos to nfnposnvel*/
	mxin = mxGetPr(prhs[10]);
	myin = mxGetPr(prhs[11]);
	mzin = mxGetPr(prhs[12]);
	for (count =0; count < nfnposnvelnb1; count++)
		{
		*mxout = *mxin++;
		*myout = *myin++;
		*mzout = *mzin++;
	...
	{
	#if DEBUG
	if (nrhs > 12) 	 /* Magnetization given, but wrong size! */
		{
...
/*TJ added dvx,dvy,dvz,nvel*/
blochsimfz(b1r,b1i,gx,gy,gz,tp,ntime,t1,t2,df,nf,dx,dy,dz,npos,dvx,dvy,dvz,nvel,db1,nb1,mx,my,mz,md);


/* ======= Reshape Output Matrices ====== */

if ((ntout > 1) && (nf > 1) && (npos > 1) && (nvel > 1) && (nb1 > 1))
{

	#if DEBUG
  		printf("Writing output, 4 output dim.\n");
	#endif

	outsize[0]=ntout;
	outsize[1]=nvel;
	outsize[2]=npos;
	outsize[3]=nf;	/*TJ velocity*/
	outsize[4]=nb1; 

	mxSetDimensions(plhs[0],outsize,5);  /* Set to 4D array. */
	mxSetDimensions(plhs[1],outsize,5);  /* Set to 4D array. */
	mxSetDimensions(plhs[2],outsize,5);  /* Set to 4D array. */

}
else if ( ( (int)(ntout>1) + (int)(nf>1) + (int)(npos>1) + (int)(nvel>1) + (int)(nb1>1)  )	== 4 )		/* Basically "squeeze" the matrix. */
{
	...
	mxSetDimensions(plhs[0],outsize,4);  /* Set to 3D array. */
	mxSetDimensions(plhs[1],outsize,4);  /* Set to 3D array. */
	mxSetDimensions(plhs[2],outsize,4);  /* Set to 3D array. */
}
else if ( ( (int)(ntout>1) + (int)(nf>1) + (int)(npos>1) + (int)(nvel>1) + (int)(nb1>1)) == 3)
{
    #if DEBUG
    printf("Writing output, 3 output dim.\n");
    #endif
	if (ntout > 1)	
	{
		if (npos > 1)
	...
	mxSetDimensions(plhs[0],outsize,3);  /* Set to 2D array. */
	mxSetDimensions(plhs[1],outsize,3);  /* Set to 2D array. */
	mxSetDimensions(plhs[2],outsize,3);  /* Set to 3D array. */
}
else if ( ( (int)(ntout>1) + (int)(nf>1) + (int)(npos>1) + (int)(nvel>1) + (int)(nb1>1)) <= 2)
{
    #if DEBUG
    printf("Writing output, 2 output dim.\n");
    #endif
	if (ntout > 1)	
	{
		outsize[0]=ntout;
