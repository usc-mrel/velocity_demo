...
if(mod(n,2)==0)
    ...
    xlfp((n+1)/2) = xlf((n+1)/2);       % keep half Nyquist the same,too % Original _ AJ xlfp((n/2+1)) = xlf((n/2+1));
    xlfp((n+1)/2+1:n) = 0*xlf((n+1)/2+1:n); % zero neg freqs % Original _ AJ xlfp((n/2+2):n) = 0*xlf((n/2+2):n)
end
xlaf = ifft(xlfp);
