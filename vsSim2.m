...
%--------------------------------------------------------------------------
% kv: integrate from  current point to the end of the pulse. 
% plot signed value of B1 and double check delta kv! should be 0.01!
% check control-label. so labeling is positive.
% color ±2 cm/sec one color scale, 15-70 cm/sec another color scale.
% background grey scale
% travis's paper (multiple cost functions)
% papers record velocity along coronaries, we only have velocity encoding
% along z. (collaterals, tortous, RCA/LCX often connect sometimes
% collateral flow from backside)
% check license for code redistribution! (rf_tools)
% optimization routine (sub-pulse ampltiudes) (try different ranges for
% blood)
% close all; clear all;
clear all;
